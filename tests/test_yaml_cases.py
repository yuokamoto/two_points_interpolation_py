...
class YAMLBasedTestRunner(unittest.TestCase):
    ...
    def _verify_trajectory_expectations(self, test_case, tpi, total_time):
        ...
        # Verify getter methods return correct values
        self.assertAlmostEqual(tpi.get_duration(), total_time, places=10,
                             msg=f"get_duration() mismatch in {name}")
        self.assertAlmostEqual(tpi.get_end_time(), tpi.t0 + total_time, places=10,
                             msg=f"get_end_time() mismatch in {name}")
